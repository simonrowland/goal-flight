...
to itself on both supported transports:
  ...
  - bash-tail: ``opencode_bash_tail.py`` + ``watch-dispatch-tail.sh``
...
Usage:
  ...
  ./scripts/opencode_self_dispatch_test.py --json
"""
...
def _run_bash_tail(*, workdir: Path, model: str, timeout: float) -> dict[str, Any]:
    worker = SCRIPT_DIR / "opencode_bash_tail.py"
...
def run_self_dispatch_test(
    *,
    ...
    if not skip_bash_tail and not (SCRIPT_DIR / "opencode_bash_tail.py").is_file():
        missing.append("opencode_bash_tail.py")
    if missing:
        return {
            "schema": SCHEMA,
            "ok": False,
