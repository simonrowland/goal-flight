...
# Grok's catalog currently exposes these levels by model family. Keep the
# fallback narrow for known families so a missing cache cannot accept an effort
# that the selected model would ignore.
GROK_DEFAULT_MODEL = "grok-4.7"
GROK_REASONING_EFFORTS_BY_MODEL = {
...
GROK_FALLBACK_REASONING_EFFORTS = frozenset().union(
    *GROK_REASONING_EFFORTS_BY_MODEL.values()
)
GROK_BASH_REASONING_AGENTS = frozenset({"grok-code", "grok-research"})
GROK_ACP_REASONING_AGENTS = GROK_BASH_REASONING_AGENTS | {"grok-acp"}


def _parse_reasoning_effort(value: str) -> str:
    """Reject an unknown level here rather than at the worker.

    Codex takes this through `-c model_reasoning_effort=...`, while Grok takes
    it through its CLI flag. Validating at the parser is what makes a typo
    fail before either worker can run at an unintended default.
    """
        ...
        if kind == "ring-seat":
            ...
            try:
                ...
                if not skip_reset or not parent_dispatch_id:
                    raise
                message = str(exc)
                if "unknown ownership" in message:
                    raise
...
def _resume_lineage_dispatch_ids(parent_dispatch_id: str) -> list[str]:
    ...
    """
    # Ledger rows are written by Goal Flight tooling. This is a consistency
    # check, not a defence against deliberate row forgery: the trusted ledger
    # writer is the boundary for parent links and branch ownership. We only
...
def _seed_codex_resume_home_from_canonical(
    ...
    *,
    ...
    try:
        if (
            built_home is None
            or built_account is None
            or (
                pre_resolved is not None
                and built_account != pre_resolved.get("account")
            )
...
def _cmd_resume(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the recorded worker reasoning effort for this resumed "
            "attempt."
        ),
    )
    parser.add_argument(
    ...
    result = None
    resume_lock = preflight["resume_lock"]
    try:
        result = main(candidate_argv, resume_plan=preflight)
    finally:
        if preflight.pop("resume_lock", None) is not None:
            resume_lock.__exit__(*sys.exc_info())
        cleanup_id = preflight.get("reserved_dispatch_id") or child_dispatch_id
        if result != 0 and not goalflight_ledger.record_path(cleanup_id).exists():
...
def _grok_reasoning_catalog(
    home: Path, model: str | None
) -> tuple[str, frozenset[str], bool]:
    """Return Grok's selected model, supported efforts, and fallback status."""
    selected_model = str(model or GROK_DEFAULT_MODEL)
    fallback_levels = GROK_REASONING_EFFORTS_BY_MODEL.get(
        selected_model, GROK_FALLBACK_REASONING_EFFORTS
    )
    try:
        cache = json.loads(
            (home / ".grok" / "models_cache.json").read_text(encoding="utf-8")
        )
        models = cache["models"]
        if not isinstance(models, dict):
            raise TypeError("Grok models cache has no model map")
        entry = models[selected_model]
        info = entry["info"]
        if not isinstance(info, dict):
            raise TypeError("Grok model entry has no info map")
        if info.get("supports_reasoning_effort") is False:
            return selected_model, frozenset(), False
        supported = info.get("reasoning_efforts")
        levels = {
            item.get("id")
        ...
        if levels:
            return selected_model, frozenset(levels), False
    except (OSError, ValueError, KeyError, TypeError, json.JSONDecodeError):
        pass
    return selected_model, fallback_levels, True


def _validate_grok_reasoning_effort(args, env: dict[str, str]) -> None:
    """Validate Grok effort against the selected account's model catalog."""
    effort = getattr(args, "reasoning_effort", None)
    if _account_engine(getattr(args, "agent", "")) != "grok" or not effort:
        return
    raw_home = env.get("HOME")
    home = Path(raw_home).expanduser() if raw_home else Path.home()
    model, levels, fallback = _grok_reasoning_catalog(
        home, getattr(args, "model", None)
    )
    if effort not in levels:
        source = (
            "; using fallback static set (models cache missing, unreadable, "
            "or no usable entry for this model)"
            if fallback
            else ""
        )
        supported = ", ".join(sorted(levels)) or "<none>"
        raise DispatchUsageError(
            f"reasoning effort {effort!r} is not supported for Grok model "
            f"{model!r}; supported levels: {supported}{source}"
        )
...
def build_worker(args, prompt_path, raw_argv: list[str]):
    ...
    if args.agent in ("grok-code", "grok-research"):
        ...
        # default applies (grok-4.7 as of 2026-09-25, per `grok models` "Default
        ...
        # flag entirely and let grok's CLI default (grok-4.7) apply.
...
def _build_launch_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--reasoning-effort", type=_parse_reasoning_effort,
                        default=None,
                        help="Codex or Grok bash reasoning effort for this dispatch "
                             f"({', '.join(sorted(CODEX_REASONING_EFFORTS))}). "
                             "Requires --agent codex|grok-code|grok-research with "
                             "--shape bash without --interactive or a raw command. "
                             "Validated against the selected model's catalog; "
                             "Grok ACP refuses this flag because its runner has no "
                             "session-options hook. Default = the worker CLI default.")
    parser.add_argument(
        "--read-only",
        "--readonly",
        action="store_true",
...
def main(argv: list[str] | None = None, *, resume_plan: dict | None = None) -> int:
    ...
    if resume_plan is None and args.worker and args.worker[0] != "--":
        word = args.worker[0]
    ...
    if getattr(args, "parent_dispatch_id", None) and resume_plan is None:
        try:
            if args.dispatch_id:
                if args.from_queue:
                    _refuse_reused_dispatch_id_for_launch(
    ...
    resume_home_path = _codex_dispatch_homes_dir() / str(args.dispatch_id)
    resume_home_created = not resume_home_path.exists() and not resume_home_path.is_symlink()
    engine_session_id = _resolved_engine_session_id(args)
    ...
    try:
        ...
        if resume_plan is not None:
            if resume_engine == "codex":
                _revalidate_codex_resume_claim(
                    ...
                    expected_home=Path(args.codex_resume_home),
                    expected_session_id=args.codex_session_id,
                    expected_home_owner_dispatch_id=args.codex_home_owner_dispatch_id,
                    exclude_dispatch_id=args.dispatch_id,
                )
            elif resume_engine and engine_session_id:
                _revalidate_resume_claim(
                    parent_dispatch_id=args.parent_dispatch_id,
        ...
        if (
            ...
            if getattr(args, "parent_dispatch_id", None):
                ...
                with (
                    ...
                    if resume_plan is not None
                    else _codex_resume_lock(resume_home, args.codex_session_id)
                ):
                    if resume_plan is None:
                        _revalidate_codex_resume_claim(
                            parent_dispatch_id=args.parent_dispatch_id,
                    ...
                    parent_record = (
                        resume_plan["source"]["record"]
                        if resume_plan is not None
                        else _find_dispatch_record(args.parent_dispatch_id) or {}
                    )
                    parent_account = parent_record.get("effective_account") or parent_record.get(
                        "account"
                    )
        ...
        if getattr(args, "parent_dispatch_id", None) and resume_engine == "codex":
            ...
            resume_lock = (
                contextlib.nullcontext()
                if resume_plan is not None
                else _engine_resume_lock(resume_engine, engine_session_id)
            )
        with resume_lock:
            if resume_plan is None:
        ...
        if not detached_launched and final_worker_alive is False:
            ...
            _export_dashboard_status_for_project(project_root)
        if (
            (codex_dispatch_home is not None or resume_home_created)
            and not detached_launched
            ...
            if (
                ...
                try:
                    if owned_home.is_symlink():
                        owned_home.unlink()
                    elif owned_home.is_dir():
                        shutil.rmtree(owned_home)
                except OSError as exc:
                    print(
                        f"goalflight_dispatch: resume home cleanup warning: {type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
            elif codex_session_id is None:
