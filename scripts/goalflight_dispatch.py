...
Easy path (agent preset — the common case):
    python3 goalflight_dispatch.py --agent codex --prompt-file p.md      # background/default; pooled worktree
    ...
    python3 goalflight_dispatch.py --agent grok-code --prompt-file p.md --at HEAD  # prepare worktree at ref
...
def _project_root(args) -> Path:
    ...
    if not configured and getattr(args, "parent_dispatch_id", None):
        parent = _find_dispatch_record(str(args.parent_dispatch_id)) or {}
        configured = parent.get("project_root")
...
def _inherited_seat_lock_present() -> bool:
    """True when this process already holds a pooled-worktree lock fd.
...
def _record_dispatch_worktree(args, lease) -> None:
    """Keep the prepared worktree's SHA, refusing unknown bases before launch."""
    try:
        ...
        raise goalflight_worktree_pool.WorktreeSeatError(
            f"dispatch refused: worktree {lease.seat_name} ({lease.path}) "
    ...
    args._worktree_reclaimed_dispatch_id = getattr(
        lease, "reclaimed_dispatch_id", None
    )
    try:
        _persist_queue_worktree_pin(args, lease, base_commit=base_commit)
    except Exception:
        # Do not leave a reset worktree live when its retry carrier could not
        # be pinned. Nearest-base selection is not equivalent.
...
def _resume_recovery_command(ref: str) -> str:
    return f"git worktree add <recovery-path> {ref}"
...
def _resume_worktree_is_descendant(
    project_root: Path, ancestor: str, descendant: str
) -> bool:
    """Accept progress committed on the dispatch's own durable branch."""
    proc = goalflight_worktree_pool._git_proc(
        project_root,
        "merge-base",
        "--is-ancestor",
...
def _resume_recovery_refs_for_parent(
    ...
    The reclaimer records the abandoned dispatch id at reclaim time. This is
    deliberately independent of checkout paths, which are reused, and reads
    cold ledger rows so archiving the reclaimer cannot hide preserved work.
    """
    refs: list[str] = []
    try:
        candidates = goalflight_ledger.read_records()
        candidates.extend(goalflight_ledger._read_archived_records())
    except OSError as exc:
        raise goalflight_worktree_pool.WorktreeCwdRefused(
            f"resume refused: could not inspect recovery refs for "
            f"{parent_dispatch_id}: {exc}"
        ) from exc
    for candidate in candidates:
        if (
            str(candidate.get("worktree_reclaimed_dispatch_id") or "")
            != parent_dispatch_id
        ):
            continue
        for key in (
...
def _resume_worktree_branch_spec(
    ...
    if recovery_refs:
        raise goalflight_worktree_pool.WorktreeCwdRefused(
            ...
            "preserved state in recovery ref(s) "
            + ", ".join(recovery_refs)
            + "; restore it before resuming with "
            + _resume_recovery_command(recovery_refs[0])
        )
    keep_ref = next(
        (
    ...
    # Repeated resumes carry the root dispatch's branch; an intermediate
    # child id is not another branch authorization.
    allowed_suffixes = {lineage_ids[-1]}
    recorded_branch = record.get("worktree_branch")
    if recorded_branch:
        recorded_branch = str(recorded_branch).strip()
        suffix = recorded_branch.split("/", 1)[1] if "/" in recorded_branch else ""
        if (
            not goalflight_worktree_pool.is_worktree_branch(recorded_branch)
            or suffix not in allowed_suffixes
        ):
            raise goalflight_worktree_pool.WorktreeCwdRefused(
                f"resume refused: recorded branch {recorded_branch} does not "
                f"belong to resume lineage of {parent_dispatch_id}"
                + _resume_worktree_ref_context(keep_ref, quarantine_ref)
            )
        candidates = [recorded_branch]
    else:
        candidates = [
            candidate
            for lineage_id in lineage_ids
            for candidate in (
                goalflight_worktree_pool.worktree_branch_name(lineage_id),
                f"seat/{lineage_id}",
            )
        ]
    ...
    if recorded_head:
        ...
        recorded_head_matches = branch_tip == resolved_head or _resume_worktree_is_descendant(
            project_root, resolved_head, branch_tip
        )
    if keep_ref:
        ...
        if branch_tip == keep_tip:
            recorded_head_matches = True
        expected_heads.append(keep_tip)
    if quarantine_ref:
        raise goalflight_worktree_pool.WorktreeCwdRefused(
            f"resume refused: preserved work for branch {branch} is in recovery ref "
            f"{quarantine_ref}; restore it before resuming with "
            f"{_resume_recovery_command(quarantine_ref)}"
        )
...
def _validate_resume_worktree_source(
    parent_dispatch_id: str, record: dict, cwd: Path
) -> None:
...
def _bind_dispatch_worktree(args) -> goalflight_worktree_pool.WorktreeSeatLease | None:
    ...
    if cwd_raw:
        ...
        if kind == "in-place" and force_captive:
            ...
            if skip_reset:
                raise goalflight_worktree_pool.WorktreeCwdRefused(
                    f"resume refused: recorded worker cwd {cwd} is not a managed "
                    "repository worktree; refusing to create or choose a "
                    "replacement worktree"
                )
            raise goalflight_worktree_pool.WorktreeCwdRefused(
                f"--cwd {cwd} is not a managed repository worktree and is not the project root. "
                "Omit --cwd to acquire a pooled worktree, or pass --in-place "
...
def _validate_before_side_effects(args, raw_argv: list[str]) -> None:
    if (
        ...
        and getattr(args, "shape", "bash") != "acp"
        and not getattr(args, "operator_recovery", False)
    ):
        raise ProvenPreWorkerRefusal(
            "--occupied-worktree-forced is deprecated and rejected for ordinary "
            "dispatch; use an operator recovery flow"
        )
    if not raw_argv:
    ...
    _resolve_launch_account_env(args)
...
def _resume_lineage_dispatch_ids(parent_dispatch_id: str) -> list[str]:
    ...
    """
    # Ledger rows are written by Goal Flight tooling, so this validates an
    # inconsistent chain rather than defending against deliberate row forgery:
    # every ancestor must exist in the same project, the chain must terminate,
    # and the root's branch must belong to the root dispatch id.
    exempt: list[str] = []
    seen: set[str] = set()
    ...
    for _ in range(_RESUME_LINEAGE_WALK_LIMIT):
        ...
        if current in seen:
            raise goalflight_worktree_pool.WorktreeCwdRefused(
                f"resume refused: inconsistent lineage cycle at {current}"
            )
        seen.add(current)
        ...
        if raw_root:
            try:
                candidate_root = goalflight_task.resolve_project_root(str(raw_root))
            except (OSError, ValueError) as exc:
                raise goalflight_worktree_pool.WorktreeCwdRefused(
                    f"resume refused: lineage ancestor {current} has an invalid project root"
                ) from exc
            if project_root is None:
                project_root = candidate_root
            elif candidate_root != project_root:
                raise goalflight_worktree_pool.WorktreeCwdRefused(
                    f"resume refused: lineage ancestor {current} belongs to a different project root"
                )
    ...
    if root_record is None:
        raise goalflight_worktree_pool.WorktreeCwdRefused(
            f"resume refused: missing lineage record for {parent_dispatch_id}"
        )
    root_id = exempt[-1]
    root_branch = str(root_record.get("worktree_branch") or "").strip()
    if root_branch:
        suffix = root_branch.split("/", 1)[1] if "/" in root_branch else ""
        if (
            not goalflight_worktree_pool.is_worktree_branch(root_branch)
            or suffix != root_id
        ):
            raise goalflight_worktree_pool.WorktreeCwdRefused(
                f"resume refused: root lineage branch {root_branch or '<missing>'} "
                f"does not belong to root dispatch {root_id}"
            )
    return exempt
...
def _refuse_existing_dispatch_id_for_resume(dispatch_id: str) -> None:
    ...
    if goalflight_ledger.record_is_unreadable(record):
        raise DispatchUsageError(
            f"resume refused: dispatch id {dispatch_id!r} already exists, "
            "but its ledger record is unreadable"
        )
    terminal = goalflight_ledger.terminal_state_for(
        record.get("state"), record.get("reason") or record.get("error")
    )
    description = (
        "a non-terminal ledger record"
        if terminal == "unknown" and record.get("state") != "watcher_stopped"
        else "a terminal ledger record"
    )
    raise DispatchUsageError(
...
def _occupancy_exempt_read_only(args) -> bool:
    ...
    if shape == "acp":
        try:
            ...
            return False
        return acp_permission_read_only_supported(agent)
    return agent in {"grok-code", "grok-research", "codex"}
...
def _release_withdrawn_worktree(record: dict | None, dispatch_id: str) -> None:
    """Best-effort stale pool-occupant release after terminal withdrawal."""
    if not isinstance(record, dict):
        return
...
def _revalidate_resume_claim(
    *,
    ...
    exclude_dispatch_id: str | None = None,
    reconcile_dead_preclaim: bool = True,
) -> None:
    ...
    *,
    ...
    try:
        if (
            built_home is None
            or built_account
            != (pre_resolved or {}).get("account", account)
...
def _cmd_resume(argv: list[str]) -> int:
    ...
    try:
        ...
        stem = _default_dispatch_id(f"{source['engine']}-resume")
        for attempt in range(1000):
            candidate_dispatch_id = stem if attempt == 0 else f"{stem}-{attempt + 1}"
            _refuse_existing_dispatch_id_for_resume(candidate_dispatch_id)
            if not (dispatch_base / ".dispatch-ids" / f"{candidate_dispatch_id}.json").exists():
                break
        else:
            raise DispatchUsageError(
                f"could not choose a resume dispatch id for stem {stem!r}"
            )
        ...
        launch_argv = preflight["launch_argv"]
        _PENDING_RESUME_PREFLIGHTS[child_dispatch_id] = preflight
    except DispatchUsageError as exc:
        print(f"goalflight_dispatch: {exc}", file=sys.stderr)
        return 64
    try:
        result = main(launch_argv)
    finally:
        _PENDING_RESUME_PREFLIGHTS.pop(child_dispatch_id, None)
    if result != 0 and not goalflight_ledger.record_path(child_dispatch_id).exists():
        reservation = dispatch_base / ".dispatch-ids" / f"{child_dispatch_id}.json"
        with contextlib.suppress(OSError):
            reservation.unlink()
    return result
...
def _resume_recorded_controller_label(record: dict) -> str | None:
    ...
    recorded = _dispatch_argv_from_record(record)
    return (
        _option_value_before_worker_remainder(
            recorded, "--controller-label", last=True
        )
        if recorded
        else None
    )
...
def _resume_launch_argv(
    ...
    *,
    ...
    recorded_reasoning_effort = record.get("reasoning_effort") or (
        _option_value_before_worker_remainder(
            recorded, "--reasoning-effort", last=True
        )
    )
    ...
    if (
        ...
        and _option_value_before_worker_remainder(
            base, "--controller-label", last=True
        ) is None
        and recorded_label
    ):
    ...
    if engine == "codex":
        ...
        # ORIGINAL home regardless of which account is billed. No migration needed.
        if requested:
        ...
        if target and owner_account and target != owner_account:
            if defer_grok_side_effects:
                # Account validation and capacity admission must finish before
                # moving files between Grok account homes.
                resume_mode = "carried"
            else:
                try:
...
_PENDING_RESUME_PREFLIGHTS: dict[str, dict] = {}
...
def _preflight_resume_dispatch(
    ...
    *,
    ...
    """Admit one resume before reserving a child id or building its home."""
    args = _build_launch_parser().parse_args(candidate_argv)
    args.task_ids = _parse_task_ids(args.tasks)
    args._original_argv = list(candidate_argv)
    _apply_fast_mode(args)
    raw = _raw_worker_args(args)
    if args.shape == "auto":
        args.shape = "acp" if args.agent in ("claude-acp", "claude") else "bash"
    if args.interactive:
        args.shape = "acp"
    ...
    if engine == "codex":
        ...
        if requested:
            ...
            parent_record = source.get("record") or {}
            ...
            if parent_account and parent_account != "default":
                ...
                selected, _rejected = select_codex_account(model=getattr(args, "model", None))
                args._capacity_account = selected
    elif engine == "grok":
    ...
    elif engine and source.get("session_id"):
    ...
    try:
        with resume_lock:
            ...
            child_dispatch_id = _reserve_auto_dispatch_id(
                f"{source['engine']}-resume", dispatch_base
            )
            _refuse_existing_dispatch_id_for_resume(child_dispatch_id)
            ...
            args.dispatch_id = child_dispatch_id
            args._resume_preflight_claim = True
            status_json = dispatch_base / f"{child_dispatch_id}.status.json"
            tail = dispatch_base / f"{child_dispatch_id}.tail"
            _record_ledger(
                args,
                project_root=project_root,
            ...
            if engine == "codex" and _CODEX_RESUME_DURABLE_CLAIM_HOOK is not None:
        ...
        if lease_id:
            _release_capacity(lease_id, "failed", "resume admission failed")
...
# account is billed. Add an engine here only with a verified layout.
...
def _codex_seat_probe_says_usable(seat: str) -> bool | None:
    ...
    measures an account exhausted and clears it when it does not -- so a fresh
...
def _preflight_resume_codex_account(args) -> None:
    ...
    Explicit resume account validation belongs before controller stamping and
    the waiting-capacity row. The resolver is the same one used by a fresh
    dispatch; its result stays on the launch namespace for the later rollout
    copy so the same attempt does not select a different account.
    """
    if (
    ...
    if source_is_canonical and str(args.account).strip() == str(parent_account or ""):
        ...
        return
    source_home = Path(str(recorded_home or args.codex_resume_home)).expanduser().resolve(
        strict=False
    )
    source_home_owner_dispatch_id = str(
        record.get("codex_home_owner_dispatch_id") or parent_dispatch_id
    )
    ...
    args._capacity_account = str(effective_account)
    args.effective_account = str(effective_account)
    args._codex_resume_source_home = str(source_home)
    args._codex_resume_source_home_owner_dispatch_id = (
        source_home_owner_dispatch_id
    )
    # The source remains available for claim validation through the private
    # source fields above; all child status/ledger rows and the worker use the
    # new home built for this resume dispatch.
    args.codex_resume_home = str(resolved_home)
    args.codex_home_owner_dispatch_id = str(args.dispatch_id)
...
def resolve_codex_home(
    ...
    *,
    ...
    skipping recently quota-exhausted accounts until their reset rather than
    dying on the first serial session. This keeps home/session claiming after
    account choice.
    ...
    print(
        ...
        f"(rejected: {rejected or 'all accounts walled or discovery failed'}); billing host",
        file=sys.stderr,
    )
...
def _prelaunch_status_metadata(
    ...
    *,
    ...
    if worktree_id:
        metadata["worktree_id"] = str(worktree_id)
        metadata["worktree_seat"] = str(worktree_id)
    if worktree_path:
    ...
    for attr, key in (
        ...
        (
            "_worktree_reclaimed_dispatch_id",
...
def _acquire_capacity(
    args, *, project_root: Path, status_json: Path | None
) -> str | None:
...
def _record_ledger(args, *, project_root: Path, prompt_path: str | None, status_json: Path,
    ...
    if state == "waiting_capacity":
        ...
        if not getattr(args, "_resume_preflight_claim", False):
        ...
        with contextlib.redirect_stdout(capture):
            code = goalflight_ledger.cmd_record(
                argparse.Namespace(
                    ...
                    worktree_reclaimed_dispatch_id=getattr(
                        args, "_worktree_reclaimed_dispatch_id", None
                    ),
                    ...
                    skip_attempt_prepare=bool(
                        getattr(args, "_resume_preflight_claim", False)
                    ),
                    state=state,
                    json=True,
                )
...
def _option_value_before_worker_remainder(
    argv: list[str], flag: str, *, last: bool = False
) -> str | None:
    ...
    try:
        ...
        if withdrawn and record and record.get("terminal_state") == terminal_state and not carriers:
            ...
            with _queue_mutation_lock(_queue_entry_path(args.dispatch_id).parent), goalflight_ledger.StateLock():
                ...
                _release_withdrawn_worktree(current, args.dispatch_id)
                try:
...
def _stamp_launch_attempt(
    ...
    *,
    ...
    back off only on a material budget burn so a free account is not parked.
    """
    stamp_fields = _apply_launch_attempt_cursor(
        claim,
        entry,
...
def _build_acp_cfg(args, *, status_json: Path, base: Path | None = None):
    ...
    acp_cwd = (
        _worker_cwd(args)
        if outer_seat_bound or explicit_cwd or getattr(args, "in_place", False)
        else project_root
    )
    if outer_seat_bound:
        acp_worktree = "off"
...
def _build_launch_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--in-place",
        action="store_true",
        help="Run in the project root without acquiring a pooled worktree.",
    )
    parser.add_argument(
        "--worktree",
        "--at",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Explicit operator recovery mode required with the deprecated "
            "--occupied-worktree-forced override."
        ),
    )
    parser.add_argument(
        "--takeover",
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = parser.parse_args(argv)
    if args.cwd and not getattr(args, "parent_dispatch_id", None):
        try:
            _existing_cwd_arg(args.cwd)
    ...
    pending_resume_preflight = _PENDING_RESUME_PREFLIGHTS.pop(
        str(args.dispatch_id or ""), None
    )
    ...
    if (
        ...
        return 64
    # Validate a direct resume before controller stamping. Reused ids must be
    # refused before any resume home can be built, including direct replay of
    # a recorded launch argv.
    if getattr(args, "parent_dispatch_id", None):
        try:
            if args.dispatch_id:
                if pending_resume_preflight is None:
                    if args.from_queue:
                        _refuse_reused_dispatch_id_for_launch(
    ...
    try:
        ...
        _refuse_launch_blocked_by_completion_authority(args)
        # Admission order is account -> capacity -> worktree -> launch. The
        # worktree bind is intentionally deferred until after the account and
        # capacity leases are durable; a refusal must not mint a checkout.
    ...
    codex_home_owner_dispatch_id = _codex_home_owner_dispatch_id(args)
    codex_resume_claim_home_owner_dispatch_id = getattr(
        args,
        "_codex_resume_source_home_owner_dispatch_id",
        codex_home_owner_dispatch_id,
    )
    if codex_home_owner_dispatch_id is not None:
    ...
    try:
        if pending_resume_preflight is not None:
            lease_id = pending_resume_preflight.get("capacity_lease_id")
            if pending_resume_preflight.get("capacity_account"):
                args._capacity_account = pending_resume_preflight["capacity_account"]
        elif (
            getattr(args, "parent_dispatch_id", None)
        ...
        if (
            ...
            and not getattr(args, "parent_dispatch_id", None)
        ):
            (
                selected_account,
        ...
        if (
            ...
            if getattr(args, "parent_dispatch_id", None):
                ...
                args._codex_resume_source_home = str(resume_home)
                args._codex_resume_source_home_owner_dispatch_id = (
                    codex_resume_claim_home_owner_dispatch_id
                )
                with _codex_resume_lock(resume_home, args.codex_session_id):
                    ...
                    _revalidate_codex_resume_claim(
                        ...
                        expected_home_owner_dispatch_id=(
                            codex_resume_claim_home_owner_dispatch_id
                        ),
                        exclude_dispatch_id=args.dispatch_id,
                    )
        ...
        if getattr(args, "parent_dispatch_id", None) and resume_engine == "codex":
            ...
            resume_lock = _codex_resume_lock(resume_home, args.codex_session_id)
            ...
            resume_lock = _engine_resume_lock(resume_engine, engine_session_id)
            resume_lock_kind = "engine"
        with resume_lock:
            if resume_lock_kind == "codex":
                _revalidate_codex_resume_claim(
                    parent_dispatch_id=args.parent_dispatch_id,
                    expected_home=resume_home,
                    expected_session_id=args.codex_session_id,
                ...
                if (
                    not pending_resume_preflight
                    and _CODEX_RESUME_CLAIM_VALIDATED_HOOK is not None
                ):
                    _CODEX_RESUME_CLAIM_VALIDATED_HOOK(
                        resume_home,
                        args.codex_session_id,
            ...
            if not pending_resume_preflight:
                _record_ledger(
                    args,
                    project_root=project_root,
                    prompt_path=prompt_path,
        ...
        if worktree_seat is not None:
            ...
            if getattr(worktree_seat, "reclaimed_dispatch_id", None):
                summary_head["worktree_reclaimed_dispatch_id"] = (
        ...
        # An auto-selected account is still an account: the API-key scrub must key on
        # the account actually resolved, or a GROK_API_KEY in the environment
        # would quietly bill the API instead of the subscription account we chose.
