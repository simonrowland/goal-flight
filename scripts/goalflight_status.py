...
def write_script_data_js(
    path: Path,
    payload: dict,
    *,
    global_name: str,
) -> None:
    """Atomically publish a JSON payload for a ``file://`` script include.

    ``global_name`` is deliberately restricted to one JavaScript identifier;
    callers supply data, never executable source.  Keeping this publisher here
    makes the existing script-safe JSON primitive a public shared boundary for
    projections without making the local status authority import them.
    """
    if re.fullmatch(r"[A-Za-z_$][A-Za-z0-9_$]*", global_name) is None:
        raise ValueError(f"invalid JavaScript global name: {global_name!r}")
    text = (
        "// generated by goalflight_status.py; do not edit by hand.\n"
...
def _write_status_data_js(path: Path, payload: dict) -> None:
    """Backward-compatible status-dashboard publisher."""
    write_script_data_js(path, payload, global_name="GF_STATUS")
