...
async function testInteractiveHistoryAndKeyedRows() {
  ...
  // The v2 history file is an append-only log of GF_HISTORY_ROWS.push lines.
  // The reader assembles the view: newest-first per project, the first row per
  // (project, dispatch) wins, a torn trailing line is skipped, and the v1
  // single-object form is still read.
  {
    const { api } = loadConsole(
      fleetPayload({ projects: [] }),
      attentionPayload({ items: [] }),
      () => Promise.reject(new Error("unused"))
    );
    const header =
      "// generated by goalflight_fleet_console_history.py; do not edit by hand.\n" +
      "// goalflight.fleet-console.history-log.v2: append-only, one record per line.\n" +
      "window.GF_HISTORY_ROWS = window.GF_HISTORY_ROWS || [];\n";
    const rec = (id, endedAt, projectId) =>
      "GF_HISTORY_ROWS.push(" + JSON.stringify({
        project_id: projectId || "p",
        name: projectId || "p",
        recorded_at: "2030-01-01T00:00:00Z",
        row: { dispatch_id: id, ended_at: endedAt },
      }) + ");\n";
    const text = header +
      rec("older", "2030-01-01T00:01:00Z") +
      rec("newer", "2030-01-01T00:05:00Z") +
      rec("older", "2030-01-01T00:09:00Z") +
      rec("other", "2030-01-01T00:02:00Z", "q") +
      'GF_HISTORY_ROWS.push({"project_id":"p","row":{"dispatch_id":"torn"';
    const view = api.parseHistoryScript(text);
    const p = view.projects.find((project) => project.project_id === "p");
    assert("v2 history log assembles newest-first, dedupes, skips a torn line", [
      view.projects.map((project) => project.project_id).join(",") === "p,q",
      p.workers.map((worker) => worker.dispatch_id).join(",") === "newer,older",
      p.workers.find((worker) => worker.dispatch_id === "older").ended_at === "2030-01-01T00:01:00Z",
    ].every(Boolean));
    const legacy = {
      schema: "goalflight.fleet-console.history.v1",
      updated_at: null,
    ...
    assert(
      "v1 single-object history is still read",
      api.parseHistoryScript("window.GF_HISTORY = " + JSON.stringify(legacy) + ";\n").projects.length === 1
    );
  }
