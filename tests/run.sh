...
_GF_TEST_ENV_BASE="$(cd "$_GF_TEST_ENV_BASE" && pwd -P)"
...
_GF_STATE_BASE="${GOALFLIGHT_STATE_DIR:-$_GF_TEST_ENV_BASE/state}"
_GF_CODEX_STATE_BASE="${GOALFLIGHT_CODEX_STATE_DIR:-$_GF_TEST_ENV_BASE/codex-state}"
_GF_WAKE_LEDGER_BASE="${GOALFLIGHT_WAKE_LEDGER_DIR:-$_GF_TEST_ENV_BASE/wake-ledger}"
_GF_FLEET_BASE="${GOALFLIGHT_FLEET_DIR:-$_GF_TEST_ENV_BASE/fleet}"
...
run_isolated_test_env() {
  ...
  env -u GOALFLIGHT_DISPATCH_ID \
    -u GOALFLIGHT_DISPATCH_SCRIPT \
    -u GOALFLIGHT_ISOLATED_TEST_FILE \
    -u GOALFLIGHT_PROJECT_ROOT \
    -u GOALFLIGHT_PROMPT_FILE \
    -u GOALFLIGHT_STEER_FILE \
    -u GOALFLIGHT_ALLOW_EXTERNAL_STEER_FILE \
    -u GOALFLIGHT_CONTROLLER_SESSION_ID \
    -u GOALFLIGHT_CONTROLLER_LEASE_NONCE \
    -u GOALFLIGHT_CONTROLLER_PID \
    -u GOALFLIGHT_CONTROLLER_LABEL \
    -u GOALFLIGHT_PROCESS_ROLE \
    -u GOALFLIGHT_LISTENER_SLOTS \
    -u GOALFLIGHT_LISTENER_LOW_WATER \
    -u GOALFLIGHT_PERSISTENT_BACKUP_SLOTS \
    ...
    -u GOALFLIGHT_WAKE_WEBHOOK_TIMEOUT_S \
    GOALFLIGHT_CAPACITY_CONF="${GOALFLIGHT_CAPACITY_CONF:-/dev/null}" \
    GOALFLIGHT_ALLOW_VOLATILE_PROJECT_ROOT="$_GF_TEST_ENV_BASE" \
    TMPDIR="$_GF_TEST_ENV_BASE" \
    GOALFLIGHT_WAKE_WEBHOOK_CONFIG="${GOALFLIGHT_WAKE_WEBHOOK_CONFIG:-/dev/null}" \
