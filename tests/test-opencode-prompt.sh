...
if [ -z "${LITELLM_API_KEY:-}" ] && [ -z "${LITELLM_MASTER_KEY:-}" ]; then
  echo "SKIP  tests/test-opencode-prompt.sh (LiteLLM env missing)"
  exit 0
fi
...
out="$("$SCRIPT" -m litellm/nano "Reply with exactly one word: pong" 2>&1)" || {
  echo "FAIL  tests/test-opencode-prompt.sh"
  echo "$out" | sed 's/^/      /'
  exit 1
}

if ! printf '%s\n' "$out" | grep -qi '^pong$'; then
  echo "FAIL  tests/test-opencode-prompt.sh (unexpected reply)"
  printf '%s\n' "$out" | sed 's/^/      /'
  exit 1
fi

echo "PASS  tests/test-opencode-prompt.sh"
