You are running a Goal Flight orchestrator behavior test for chat-as-requirements.
...
Per `SKILL.md` "Orchestrator chat is requirements input, not an inline editor
command": what do you do?

Reply as the orchestrator. State that, in a real run, the asks should be queued
