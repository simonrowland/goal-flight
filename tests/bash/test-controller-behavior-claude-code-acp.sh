...
SCENARIOS="${GOALFLIGHT_CONTROLLER_SCENARIOS:-doctor-loads resume-after-compaction continue-prescribed-step-two read-skill-end-to-end compaction-reload-skill compaction-reload-in-skill-continuation review-flight-at-completion chat-as-requirements draft-goal-office-hours vague-goal-premise-backlog context-load-order goal-loop-default dispatch-cli-worker-via-crash-safe-command never-pgrep-for-worker-liveness no-hand-iterate}"
FAIL=0
