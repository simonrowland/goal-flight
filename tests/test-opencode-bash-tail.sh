...
if ! command -v opencode >/dev/null 2>&1; then
  echo "SKIP  tests/test-opencode-bash-tail.sh (opencode not installed)"
  exit 0
fi

if ! command -v python3 >/dev/null 2>&1; then
  echo "SKIP  tests/test-opencode-bash-tail.sh (python3 missing)"
  exit 0
fi
...
if [ -z "${LITELLM_API_KEY:-}" ] && [ -z "${LITELLM_MASTER_KEY:-}" ]; then
  echo "SKIP  tests/test-opencode-bash-tail.sh (LiteLLM env missing)"
  exit 0
fi
...
if ! wait "$WORKER_PID"; then
  echo "FAIL  tests/test-opencode-bash-tail.sh (worker exited non-zero)"
...
if ! wait "$WATCHER_PID"; then
  echo "FAIL  tests/test-opencode-bash-tail.sh (watcher exited non-zero)"
...
if ! grep -q '^COMPLETE: true$' "$TAIL"; then
  echo "FAIL  tests/test-opencode-bash-tail.sh (missing COMPLETE marker)"
  cat "$TAIL" | sed 's/^/      /'
  exit 1
fi

if ! grep -Eq '^[^C].*4|^4$' "$TAIL"; then
  echo "FAIL  tests/test-opencode-bash-tail.sh (expected reply 4 in tail)"
  cat "$TAIL" | sed 's/^/      /'
  exit 1
fi

echo "PASS  tests/test-opencode-bash-tail.sh"
