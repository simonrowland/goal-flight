...
def test_opt_out_still_exists_and_is_described_by_its_consequence() -> None:
    ...
    # argparse WRAPS help text, so match on whitespace-normalised output. An
    # earlier version compared against the raw string and broke the moment the
    # wording shifted a few characters and the phrase spanned two lines.
    help_text = " ".join(_help().split())
