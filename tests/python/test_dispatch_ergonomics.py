...
def test_moonshot_worker_argv() -> None:
    with tempfile.TemporaryDirectory() as td:
        prompt = Path(td) / "prompt.md"
        prompt.write_text("Implement the change.\n", encoding="utf-8")
        args = _args(agent="moonshot", cwd="/tmp/x")
        argv, stdin_path = D.build_worker(args, prompt, [])
        joined = " ".join(argv)
        check("moonshot uses login-shell cwd wrapper", argv[:2] == ["/bin/sh", "-lc"] and 'cd "$0"' in argv[2])
        check("moonshot resolves kimi off-PATH fallback", 'command -v kimi' in argv[2] and '$HOME/.kimi-code/bin/kimi' in argv[2])
        check("moonshot uses print text mode", ' -p "$prompt" --output-format text ' in argv[2])
        check("moonshot omits incompatible permission flags", "--auto" not in joined and "-y" not in argv and "--yolo" not in argv)
        check("moonshot passes cwd as shell arg and add-dir", argv[3] == "/tmp/x" and argv[-2:] == ["--add-dir", "/tmp/x"])
        check("moonshot passes prompt by argv", argv[4] == "Implement the change.\n" and stdin_path is None)
        check("moonshot omits model by default", "--model" not in argv)

        args.model = "kimi-code/k3-preview"
        model_argv, _ = D.build_worker(args, prompt, [])
        check("moonshot passes explicit model", model_argv[-4:] == ["--model", "kimi-code/k3-preview", "--add-dir", "/tmp/x"])
        ...
        os.chdir(repo)
        try:
            relative_args = _args(agent="moonshot", cwd="tests")
            resolved_relative_cwd = os.path.abspath("tests")
            relative_argv, _ = D.build_worker(relative_args, prompt, [])
        finally:
            os.chdir(original_cwd)
        check(
            "moonshot resolves relative cwd once for shell and add-dir",
            relative_argv[3] == resolved_relative_cwd
            and relative_argv[-2:] == ["--add-dir", resolved_relative_cwd],
        )

    check("moonshot single-account design omits account engine", D._account_engine("moonshot") is None)
    try:
        D._resolve_account_env(_args(agent="moonshot", account="alternate"))
    except D.DispatchUsageError as exc:
        check(
            "moonshot named account is rejected before profile resolution",
            "--account is not configured for --agent 'moonshot'" in str(exc),
        )
    else:
        check("moonshot named account is rejected before profile resolution", False)

    for sandbox_args in (
        _args(agent="moonshot", read_only=True, os_sandbox=None),
        _args(agent="moonshot", read_only=False, os_sandbox="workspace-write"),
    ):
        try:
            D._validate_agent_os_sandbox(sandbox_args)
        except D.DispatchUsageError as exc:
            check(
                f"moonshot rejects unenforced sandbox {D._effective_os_sandbox(sandbox_args)}",
                "supports only --os-sandbox off" in str(exc) and "b-079" in str(exc),
            )
        else:
            check(f"moonshot rejects unenforced sandbox {D._effective_os_sandbox(sandbox_args)}", False)
    try:
        D._validate_agent_os_sandbox(_args(agent="moonshot", os_sandbox="off"))
    except D.DispatchUsageError as exc:
        check(f"moonshot accepts explicit sandbox off ({exc})", False)
    else:
        check("moonshot accepts explicit sandbox off", True)
    check(
        "moonshot default sandbox posture records off",
        D._effective_os_sandbox(_args(agent="moonshot", os_sandbox=None)) == "off",
    )
    try:
        ...
        check(
            "moonshot read-only dispatch is rejected before side effects",
            "requested profile 'read-only' is not enforced" in str(exc),
        )
    else:
        check("moonshot read-only dispatch is rejected before side effects", False)


def test_moonshot_worker_dash_execution() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        args = _args(agent="moonshot", cwd=str(cwd), model="m")
        argv, stdin_path = D.build_worker(args, prompt, [])
        env = os.environ.copy()
