...
skip_posix_on_native_windows("fleet steering fixtures use POSIX /tmp paths")
...
if __name__ == "__main__":
    ...
    if failed:
        print(f"FAIL tests/python/test_fleet_steering.py ({len(failed)} failed of {passed + len(failed)})")
        for name, err in failed:
            print(f"  - {name}: {err}")
        sys.exit(1)
    print(f"PASS tests/python/test_fleet_steering.py ({passed} tests)")
