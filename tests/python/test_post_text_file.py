...
def test_junk_drawer_types_bounce_with_controller_notice_syntax(
    ...
    assert posted.returncode == 2, posted.stdout
    # The refusal names the offending type and the working form. The exact
    # sentence is the addressee gate's (PR #12), which refuses the same junk
    # types earlier and says why: "type 'x' is not delivered as controller
    # mail; mail with no addressee is not delivered." Assert the content, not
    # one wording, so either gate satisfies it.
    assert f"type {junk!r}" in posted.stderr
    assert "not delivered" in posted.stderr or "not a sendable post type" in posted.stderr
