...
python3 - "$CLAUDE_MANIFEST" "$CODEX_ROOT_MANIFEST" "$CODEX_PLUGIN_MANIFEST" "$CODEX_MARKETPLACE" "$VERSION_FILE" <<'PY'
import json
import sys
from pathlib import Path

manifest_paths = [Path(value) for value in sys.argv[1:4]]
