...
if ! command -v python3 >/dev/null 2>&1; then
  echo "SKIP  tests/test-opencode-self-dispatch.sh (python3 missing)"
  exit 0
fi
...
if [ -z "${LITELLM_API_KEY:-}" ] && [ -z "${LITELLM_MASTER_KEY:-}" ]; then
  echo "SKIP  tests/test-opencode-self-dispatch.sh (LiteLLM env missing)"
  exit 0
fi
...
if ! python3 "$SCRIPT" --directory "$REPO_ROOT" --json > /tmp/opencode-self-dispatch-$$.json 2>/tmp/opencode-self-dispatch-$$.err; then
  echo "FAIL  tests/test-opencode-self-dispatch.sh"
...
print("PASS  tests/test-opencode-self-dispatch.sh")
PY
