...
| `task-lifecycle.md` | task/store behaviour, `tasks.jsonl`, generated task/bug views, dispatch status joins, or `capture` / `list` / `next` doctrine |
...
| `dispatch-resume.md` | resume the dead worker or dispatch a fresh one: keep its context or buy a clean read |
...
| `maintenance.md` | installing or operating the standalone hourly artifact-retention job |
