...
with that path. An unreadable or unresolvable registry is UNKNOWN and does not record a
reached-nobody delivery. Every non-zero `--to-controller` exit emits that object on
stdout (a usage refusal is `controller_post_usage` and is not a miss).
