...
Run the bundled structured review helper as a closeout check (code review, not approval routing).
...
- If the blamed PR was merged by an automation/bot account or another automation, identify the human trigger when practical. Check timeline/comments first; if rate-limited, use a GitHub cache or public PR HTML. Look for maintainer commands such as `@<bot> automerge`, `/landpr`, or labels/status comments that armed automerge. Report `automerge triggered by @login`; if not found, say trigger unknown.
...
```bash
autoreview/scripts/autoreview --mode commit --commit HEAD
```
...
```bash
autoreview/scripts/autoreview --parallel-tests "<focused test command>"
```
...
Vendored helper in the goal-flight repository:

```bash
autoreview/scripts/autoreview --help
```

Goal Flight wraps this helper via `scripts/autoreview.sh` (default engine and Claude ACP routing).

On native Windows, invoke the extensionless Python helper through Python:

```powershell
python autoreview\scripts\autoreview --help
```

The smoke harness has thin shell wrappers over a shared Python implementation:

```bash
autoreview/scripts/test-review-harness --fixture benign --engine codex
```

```powershell
autoreview\scripts\test-review-harness.ps1 -Fixture benign -Engine codex
```
