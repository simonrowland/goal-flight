...
HARVEST_PLACEHOLDER_RE = re.compile(r"<[^>\n]+>")
...
def _harvest_title_key(value: Any) -> str:
    text = html.unescape(_clean_markdown_title(str(value or "")))
...
def _harvest_has_placeholder(value: Any) -> bool:
    return bool(HARVEST_PLACEHOLDER_RE.search(html.unescape(str(value or ""))))
...
def _skip_harvest_source(path: Path, text: str) -> bool:
    name = path.name
    if name in HARVEST_MANAGED_SOURCE_NAMES or path.suffix.lower() == ".html":
        return True
    if name == "bug-backlog.md" and HARVEST_GENERATED_HEADER in text:
        return True
