...
- **Design checkpoint — plan first, build after approval.** If this chunk decides interfaces, invariants, data shapes, or acceptance criteria (anything where a wrong choice costs a rebuild rather than an edit), your FIRST deliverable is the design, not the code. Write it to the lane's research directory — interfaces, invariants, acceptance checks, the test names you intend — then emit `!USER-NEED: design ready for approval at <path>` and STOP. Do not start building. The controller reviews and steers back approval or corrections, and a wrong design then costs one steer instead of a whole build. Skip this only for chunks with no design content: a mechanical edit, a rename, a fix whose shape the brief already fixes. When unsure whether a chunk qualifies, checkpoint — an unnecessary checkpoint costs one round trip, a skipped one costs the build.
...
- After `!USER-CONFIRM:`, **do not take the guarded action through ordinary ACP
  auto permissions**, even after a correlated `yes`. The answer records consent
  for audit but cannot identify the later tool call, so non-read permissions
  stay closed for the remainder of this ACP connection. Use inline permission
  mode or a new explicitly authorized dispatch for that action. A steer ACK,
  unrelated steer, mailbox read, timeout, or silence is not approval. Continue
  independent safe read-only work while the question is outstanding. At the
  unattended deadline the harness reconciles a correlated affirmative whose
