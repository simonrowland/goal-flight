...
Set `timeout_ms` to the host maximum. Claude Code caps a monitor at 30 minutes and has no persistent option; on expiry the controller is deaf until it re-arms `supervise`, which renews the lease before arming. The older decomposed fallback's `dead_after_s=360` threshold is evaluated only when a surviving watchdog/listener reads durable monitor state; it is not a background timer. If the host has reaped every tracked task, a manual `follow`/`listen` pass is what resumes that observation, so the 30-minute cap plus the six-minute threshold can look like roughly 45 minutes of deafness. The supervisor slot records PID/start-token identity, so a dead or recycled owner is reclaimed immediately. A live owner is never displaced. If the process probe is unknown, the refusal says coverage was not verified; run `--list-controllers`, or use `supervise --takeover` only when the recorded PID and start token prove the owner is dead.
