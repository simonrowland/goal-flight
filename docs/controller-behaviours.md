...
and frontiers; `--debug`
restores per-tick heartbeat and coverage. Set `timeout_ms` to the host maximum. Claude Code caps a monitor at 30 minutes and has no persistent option; on expiry the controller is deaf until it re-arms `supervise`, which renews the lease before arming. The older decomposed fallback's `dead_after_s=360` threshold is evaluated only when a surviving watchdog/listener reads durable monitor state; it is not a background timer. If the host has reaped every tracked task, a manual `follow`/`listen` pass is what resumes that observation, so the 30-minute cap plus the six-minute threshold can look like roughly 45 minutes of deafness. The supervisor slot records PID/start-token identity, so a dead or recycled owner is reclaimed immediately. A live owner is never displaced. If the process probe is unknown, the refusal says coverage was not verified; run `--list-controllers`, or use `supervise --takeover` only when the recorded PID and start token prove the owner is dead. A persistent (unbounded) monitor can also be
...
re-emission by envelope identity and names the stuck child (`cursor-lag` /
`child-backlog`); distinct envelopes still forward, and `distinct-withheld`
points at `relay --drain` when distinct volume itself is the risk. In the decomposed fallback, only after supervisor absence is
...
that monitor, never shell `&`, and keep two tracked `listen --listener-slots 2
...
`listener-dead` on stdout. In the decomposed unsupervised path, that record and a
`watchdog-dead` record carry the exact component re-arm command. Under `supervise`,
they keep the reason but omit the component action; recovery is a supervisor restart.
The backup checks the watchdog lock during its existing wait and emits
`watchdog-dead` after a missing lock;
this witnesses lone watchdog death and stream-plus-watchdog death, but not the backup's
own death or host-reaping of every tracked task. Together they are persistent coverage
`live/4`, detectable but not reap-proof.
...
- **controller_does:** The orchestrator auto-claims without stealing a live different lease, prefers one `supervise` feed that owns the persistent set and sets `timeout_ms` to the host maximum (Claude Code caps a monitor at 30 minutes and has no persistent option; re-arming `supervise` renews the lease before arming), and only after supervisor absence is proven arms `follow` through a persistent stdout monitor where supported, arms a two-doorbell backup pool plus one independently locked `--watch-follow` watchdog that reads durable record age and emits channel death on stdout after three missed beats, lets the backup witness a missing watchdog lock while stating that all-tracked-task death remains unwitnessed, treats stream, backup pool, and watchdog as persistent coverage `live/4`, and retains the portable tracked-listener pool on hosts without a monitor.
