...
healthy child start/stop stay off stdout. A changed frontier still becomes
one `kind=next` record; a verbatim-identical payload is suppressed until the
15-minute floor. `--chatty` restores raw keepalives and frontiers; `--debug`
...
points at `relay --drain` when distinct volume itself is the risk. On Claude Code use `persistent: true`; that
