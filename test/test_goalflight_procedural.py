...
def test_doctor_json_shape() -> None:
    ...
    assert_true("opencode host install probe", "opencode" in payload["host_goalflight_install"])
    assert_true("opencode section", "opencode" in payload and "config" in payload["opencode"])
    assert_true("opencode context-mode section", "opencode_context_mode" in payload)
    assert_true("opencode acp worker probe", "opencode-acp" in payload["acp"])
