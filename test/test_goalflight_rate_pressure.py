...
if __name__ == "__main__":
    ...
    if failed:
        print(f"FAIL  test/test_goalflight_rate_pressure.py ({len(failed)} failed of {passed + len(failed)})")
        for name, err in failed:
            print(f"  - {name}: {err}")
        sys.exit(1)
    print(f"PASS  test/test_goalflight_rate_pressure.py ({passed} assertions)")
