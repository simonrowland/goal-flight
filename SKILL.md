...
- **Never wrap `codex exec` (or `grok -p`, `claude -p`) inside an MCP tool call** like `ctx_execute` or `ctx_batch_execute`. Context-mode wraps shell calls in an FTS5 sandbox subject to MCP timeouts; long headless dispatches (≥ tens of seconds, which is most useful codex / grok / claude review or /goal-mode invocations) hit the MCP/context timeout before the dispatch completes — the controller sees a hang, but the underlying process ran fine and exited normally. The output is just inaccessible. Run headless dispatches via Bash with `>` redirection to a file, poll the file or use a process-status watcher (`while kill -0 $PID; do sleep 15; done`), and `ctx_search` / `ctx_execute` over the captured output **AFTER** the dispatch exits. Canonical capture upstream: `montecarlo/AGENTS.md` §GStack Claude Reviews (`claude -p` adversarial review pattern). Failure-mode symptom: the controller appears to stall on a single tool call with no streaming output for minutes; if you see this, kill the wrapping MCP call and re-dispatch via Bash + redirection.
