...
- **Antipattern: `claude -p`.** It consumes Anthropic API billing rates instead of your session billing. **Always use the Agent tool** to spawn Claude subagents (their work is part of your session billing). Codex reviewers go through `Bash timeout 300 codex exec --ignore-user-config '...'` (see `reference/pattern.md` §Codex reliability — `--ignore-user-config` prevents the MCP approval-gate stall that otherwise breaks ~2/5 dispatches; `timeout` enforces a hard ceiling since codex v0.130.0 has no `--timeout` flag).
...
- **Leverages [gstack](https://github.com/garrytan/gstack)** when installed — Gary Tan's skill pack works for **both Claude Code and codex**, exposing `/review`, `/office-hours`, `/plan-eng-review`, `/plan-ceo-review`, `/cso`, `/investigate`, etc. Init checks both install locations (`~/.claude/skills/gstack/`, `~/.codex/skills/gstack/`, plus project-level `.agents/skills/gstack/`) and recommends install if either side is missing. Commands prefer **Claude-direct invocation** (Skill tool, e.g., `/review <range>`) when available; use `timeout 300 codex exec --ignore-user-config '/review <range>'` for the parallel second-opinion perspective (see `reference/pattern.md` §Codex reliability for the dispatch shape rationale); fall back to local prompts in `prompts/` only when gstack is absent on both sides.
