...
- **Token bias defaults UP** — largest model + highest reasoning + parallel reviewers + corpus eagerly built — because subagent / codex / grok tokens are cheap relative to engineering quality; override DOWN per-chunk via `[controller-direct]` or a smaller `model:` field for trivially-small work. The dispatch *channel* (Opus / codex / grok) is a user-direction call, not a skill prescription — accept routing preferences at session start (typical: codex for coding when Claude session-limits bite from concurrent runs, Claude for reviews and orchestration, `grok -p` for parallel second-opinion review sweeps) and apply them consistently through the run.
