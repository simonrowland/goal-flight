...
**Polling shape — Bash `&` + tail-file workers** (codex `exec`, `grok -p` / `grok --prompt-file`, anything launched via Bash `&` with output redirected to a tail file): the watcher (`while kill -0 $PID; do sleep 15; done` via `run_in_background: true` Bash) fires a task-notification on worker exit. The controller reads the tail file, greps for marker lines in order — **tolerating optional markdown emphasis** around the marker tag (grok wraps markers as `**STATUS:** ...`; codex emits unwrapped `STATUS: ...`; pattern `^\**(STATUS|RESULT|USER-NEED|USER-CONFIRM|BLOCKED|COMPLETE):\**`) — and processes each. The `COMPLETE:` marker is what tells the controller the chunk's work is done (vs. a `BLOCKED:` or `USER-NEED:` exit).
