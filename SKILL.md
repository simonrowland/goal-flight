...
- **Antipattern: `claude -p`.** It consumes Anthropic API billing rates instead of your session billing. **Always use the Agent tool** to spawn Claude subagents (their work is part of your session billing). Codex reviewers go through `Bash timeout 300 codex exec '...'`. `timeout` enforces a hard ceiling since codex v0.130.0 has no `--timeout` flag. The MCP approval-gate stall that otherwise breaks ~2/5 dispatches is solved at init time by `scripts/install-codex-overrides.sh`, which registers the project (and its worktrees by path prefix) as codex-trusted. See `reference/pattern.md` §Codex reliability for full background and the `--ignore-user-config` fallback when the trust registration isn't an option.
